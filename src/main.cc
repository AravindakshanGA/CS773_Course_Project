...
void swap_context(uint8_t swap_cpu_0, uint8_t swap_cpu_1)
{
 ...
 void print_backreq_stats() {
     ...
     for (int i = 0; i < NUM_CPUS; i++) {
         cout << "\nBack-invalidation requests for CPU " << i << endl;
         cout << "\n#evictions in LLC: " << ooo_cpu[i].LLC_eviction_counter << endl;    
         cout << "\n#back-invalidation requests in L1: " << ooo_cpu[i].L1_back_req_counter << endl;
         cout << "\t#back-invalidation requests in L2: " << ooo_cpu[i].L2_back_req_counter << endl;
     }
